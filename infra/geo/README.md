...
> ## ⚠️ 이 폴더는 이제 기록입니다 (2026-09-16)
>
> **sia.haus 는 아임웹을 떠나 `studio/` 의 정적 사이트로 갑니다.** 아래 내용은
> 아임웹에 남아 있을 때의 진단과 전략이며, **더 이상 실행 지시가 아닙니다.**
> 여기 적힌 "아임웹 관리자에서 …" 절차를 따라 하지 마세요.
>
> 남겨 두는 이유는 **왜 옮겼는지**가 여기 있기 때문입니다. 아임웹이 자동으로 넣는
> 마크업을 우리가 못 고친다는 것, 그래서 `VideoObject` 오류를 손댈 수 없었다는 것이
> 이사의 직접적인 근거입니다.
...
| 이 폴더의 파일 | 지금 | 상태 |
|---|---|---|
| `imweb-head-snippet.html` | `studio/build.py` 의 `build_jsonld()` | **삭제됨** — 붙여넣을 곳이 없어졌습니다 |
| `llms.txt` | `studio/llms.txt` (빌드 산출물) | 출처 기록으로 남김 |
| `page-copy-draft.md` | `studio/data/site.json` | 출처 기록으로 남김 |
| `README.md` (이 파일) | — | 진단 기록으로 남김 |

`studio/data/site.json` 이 이 폴더의 두 파일을 **값의 출처로 인용**하고 있어
지우지 않았습니다. 지우면 "이 문장이 어디서 왔는가"를 확인할 수 없게 됩니다.

## 해소된 미결 사항

| 항목 | 당시 | 지금 |
|---|---|---|
| canonical 도메인 | 미정 | ✅ `https://www.sia.haus/` (2026-08-21) |
| 구독 라이선싱 출시 여부 | 미확인 | ✅ **출시 전** (2026-09-16 확인) |

출시 전이므로 사이트의 정의문은 "작가 IP로 축적하고 있습니다" 로 나가고
라이선싱 FAQ 는 빠집니다. `studio/data/site.json` 의 `licensing.launched` 가
이 결정을 담고 있으며, 출시하면 `true` 로 바꾸면 됩니다.
...
> 🚫 **아래 절차는 폐기됐습니다.** 아임웹을 떠나므로 실행하지 마세요.
> `imweb-head-snippet.html` 은 삭제됐고, 같은 일을 `studio/build.py` 가 합니다.
> 남겨 두는 이유는 순서에 담긴 판단 — **본문 노출이 FAQPage 보다 먼저** — 이
> 여전히 유효하기 때문입니다. `studio` 는 둘을 같은 데이터에서 만들어 이 순서를
> 구조적으로 보장합니다.
...
> **정의문의 "라이선싱합니다" 는 서비스가 실제로 출시된 뒤에만 참입니다.**
> 그 전에 올리면 없는 서비스를 설명하게 됩니다.
>
> **2026-09-16 확인 — 아직 출시 전입니다.** 그래서 `studio` 는 "작가 IP로
> 축적하고 있습니다" 로 끝나는 문안을 쓰고 라이선싱 FAQ 를 뺍니다. `license/`
> 사이트도 출시 전에는 배포하지 마세요 — 없는 서비스의 판매 페이지가 됩니다.
> 출시하면 `studio/data/site.json` 의 `licensing.launched` 를 `true` 로 바꾸면
> 정의문과 FAQ 가 함께 복구됩니다.
